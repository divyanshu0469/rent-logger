...
const Page = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center items-center gap-2">
        ...
        {data?.tenants
          ...
          .map((tenant) => {
            return (
              ...
              >
                <AccordionItem
                  value={`tenant-${tenant._id.toString()}`}
      ...
      <h3 className="font-bold text-3xl p-4 text-gray-400">Archived</h3>
      <Accordion
        type="single"
        collapsible
        className="w-1/2 max-2xl:2/3 max-xl:w-4/5 max-md:w-full flex flex-col justify-center items-center gap-2"
        ...
        {data?.tenants
          ...
          .map((tenant) => {
            return (
              <div
                key={tenant._id.toString()}
                className="w-full p-2 flex flex-row justify-start items-center"
              >
                <AccordionItem
                  value={`tenant-${tenant._id.toString()}`}
                  className="w-11/12 p-2 rounded-md text-gray-400 bg-gray-50"
                >
                  <AccordionTrigger className="text-xl font-bold">
                    {tenant.name}
                  </AccordionTrigger>
